...
*Setup instructions for a gitian build of CBSL using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the CBSL
...
guide. It is preferred to follow these stCBSL yourself instead of using someone else's
...
At the end the VM is rebooted to make sure that the changes take effect. The stCBSL in this
...
The rest of the stCBSL in this guide will be performed as that user.
...
Clone the git repositories for CBSL and gitian and then checkout the CBSL version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/CBSL-crypto/CBSL.git
cd CBSL
...
Follow the instructions in [doc/release-process.md](release-process.md) in the CBSL repository
...
To build CBSL (for Linux, OSX and Windows) just follow the stCBSL under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the CBSL repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/CBSL/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/CBSL-crypto/CBSL
...
For example:
```bash
URL=https://github.com/CBSLadnetwork/CBSL.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit CBSL=${COMMIT} --url CBSL=${URL} ../CBSL/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit CBSL=${COMMIT} --url CBSL=${URL} ../CBSL/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit CBSL=${COMMIT} --url CBSL=${URL} ../CBSL/contrib/gitian-descriptors/gitian-osx.yml
```
...
and follow the stCBSL in the build process as normal.

    gpg: skipped "CBSLcoin": secret key not available

When you execute `gsign` you will get an error from GPG, which can be ignored. Copy the resulting `.assert` files
in `gitian.sigs` to your signing machine and do

```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/CBSL-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/CBSL-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/CBSL-build.assert
```
...
[CBSL/gitian.sigs](https://github.com/CBSL-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
