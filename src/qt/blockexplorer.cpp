...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (CBSL.conf).");
            QMessageBox::warning(this, "CBSL Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
