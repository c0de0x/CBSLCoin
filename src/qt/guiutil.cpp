...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a CBSLCoin address (e.g. %1)").arg("CTxoCJjWBU7KNNLE5vfwitf5NTmBH1xoWR"));
#endif
...
{
    // return if URI is not valid or is no CBSL: URI
...
{
    // Convert CBSL:// to CBSL:
    //
    //    Cannot handle this later, because CBSL:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open CBSL.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "CBSL.lnk";
}
...
{
    return GetAutostartDir() / "CBSL.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a CBSL.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=CBSL\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the CBSL app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add CBSL app to startup item list
