...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeCBSLAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeCBSLAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeCBSLAmount = strAnonymizeCBSLAmount.remove(strAnonymizeCBSLAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCBSLAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeCBSLAmount * COIN) nMaxToAnonymize = nAnonymizeCBSLAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeCBSLAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeCBSLAmount));
        strAnonymizeCBSLAmount = strAnonymizeCBSLAmount.remove(strAnonymizeCBSLAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCBSLAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
