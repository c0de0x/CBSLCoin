...
// Copyright (c) 2019 CBSLCoin Core Developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#if defined(HAVE_CONFIG_H)
#include "config/CBSL-config.h"
#endif
...
/** Preparing stCBSL before shutting down or restarting the wallet */
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "CBSL.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "CBSLd.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 28864, 28865));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, CBSL, (obfuscation, swifttx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-logCBSL", strprintf(_("Include IP addresses in debug output (default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all CBSL specific functionality (Masternodes, Obfuscation, SwiftTX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:28864"));
    ...
    strUsage += HelpMessageOpt("-anonymizeCBSLamount=<n>", strprintf(_("Keep N CBSL anonymized (default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 28864, 28865));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The CBSL Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that CBSL is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    fLogIPs = GetBoolArg("-logCBSL", false);
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. CBSL is shutting down."));
...
#endif
    // Make sure only a single CBSL process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. CBSL is probably already running."), strDataDir));
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of CBSL") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart CBSL to complete") << "\n";
    ...
    nAnonymizeCBSLAmount = GetArg("-anonymizeCBSLamount", 0);
    if (nAnonymizeCBSLAmount > 999999) nAnonymizeCBSLAmount = 999999;
    if (nAnonymizeCBSLAmount < 2) nAnonymizeCBSLAmount = 2;
    ...
    LogPrintf("Anonymize CBSL Amount %d\n", nAnonymizeCBSLAmount);
    ...
    /* Denominations
       ...
       For example:
       1CBSL+1000 == (.1CBSL+100)*10
       10CBSL+10000 == (1CBSL+1000)*10
    */
