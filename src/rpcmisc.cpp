...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"obfuscation_balance\": xxxxxx, (numeric) the anonymized cbslcoin balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in cbslcoin/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in cbslcoin/kb\n"
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"cbslcoinaddress\"\n"
            "\nReturn information about the given cbslcoin address.\n"
            "\nArguments:\n"
            "1. \"cbslcoinaddress\"     (string, required) The cbslcoin address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"cbslcoinaddress\", (string) The cbslcoin address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: CbslCoin address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are cbslcoin addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) cbslcoin address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
